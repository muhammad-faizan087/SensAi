...
const Quiz = () => {
  ...
  if (resultData) {
    return (
      <div className="mx-2">
        <QuizResult result={resultData} StartNew={startNewQuiz} />
      </div>
    );
  }
  ...
  if (!quizData) {
    return (
      <Card className="mx-2 space-y-4 bg-background">
        <CardHeader>
  ...
  const finishQuiz = async () => {
    ...
    try {
      await savingQuizFn(quizData, Answers, score);
  ...
  return (
    <Card className="mx-2 ">
      <CardHeader>
        <CardTitle>
          Question {questionNo + 1} of {quizData.length}
        </CardTitle>
      </CardHeader>
      ...
      <CardFooter className="flex flex-col sm:flex-row sm:justify-between gap-2">
        ...
        >
          ...
          <span>
            {questionNo < quizData.length - 1 ? "Next Question" : "Finish Quiz"}
          </span>
        </Button>
      </CardFooter>
    </Card>
